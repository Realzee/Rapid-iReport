...
const ReportDetailCard: React.FC<ReportDetailCardProps> = ({ report, onClose, profile, onEdit, onDelete, onViewOnMap, allUsers }) => {
    ...
    const isRecoveredOrDeleted = localReport.status === 'recovered' || localReport.status === 'deleted' || localReport.status === 'resolved' || localReport.status === ReportStatus.RECOVERED || localReport.status === ReportStatus.DELETED || localReport.status === ReportStatus.RESOLVED;
            ...
            <div className={`flex flex-col flex-grow ${isRecoveredOrDeleted ? 'opacity-40 grayscale blur-[0.5px]' : ''}`}>
                <div className="flex justify-between items-start mb-4 flex-shrink-0 gap-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white truncate flex-1 min-w-0 pr-2">{localReport.type === 'vehicle' ? (localReport as any).license_plate : localReport.title}</h3>
                    </div>
