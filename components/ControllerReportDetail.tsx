    ...
    return (
        <div className="bg-gray-900/80 border border-gray-800 rounded-2xl h-full flex flex-col text-white backdrop-blur-lg">
            ...
            <div className="p-4 flex-grow overflow-y-auto">
                <h4 className="font-bold text-gray-300 mb-2">Live Feed</h4>
            ...
            <form onSubmit={handleUpdateSubmit} className="p-4 flex-shrink-0">
                 <div className="relative">
                    <input
                        ...
                        className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 pl-3 pr-10 text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                    <button type="submit" disabled={isSubmittingUpdate} className="absolute inset-y-0 right-0 px-3 flex items-center text-blue-400 hover:text-blue-300 disabled:opacity-50">
            ...
            <div className="p-4 border-t border-gray-700/50 space-y-4 flex-shrink-0">
                <div>
                    <label className="text-sm font-semibold text-gray-400 mb-1 block">Update Status</label>
                    <select
                        value={report.status}
                        onChange={(e) => handleStatusUpdate(e.target.value as ReportStatus)}
                        className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-1 focus:ring-blue-500 capitalize"
                    >
                        {statusOptions.map(s => <option key={s} value={s}>{s.replace('_', ' ')}</option>)}
                    </select>
                </div>
                 <div>
                    <label className="text-sm font-semibold text-gray-400 mb-1 block">Dispatch Responder</label>
                    <select
                        value={report.assigned_to || ''}
                        onChange={(e) => handleDispatchResponder(e.target.value)}
                        className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                    >
                        <option value="">{report.assigned_to ? 'Unassign' : 'No responders available'}</option>
