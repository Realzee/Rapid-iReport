...
const ControllerReportDetail: React.FC<{ 
    ...
    const generateBoloImage = async (action: 'download' | 'share' = 'download') => {
        ...
        try {
            ...
            ctx.fillText(companyName, width / 2, 30);
            ...
            ctx.fillText(truncatedHeader, width / 2, 75);
            ...
            if (customBgImg) {
                ctx.save();
                ctx.beginPath();
                ctx.rect(0, 380, width, height - 380);
                ctx.clip();
                
                const bgRatio = customBgImg.width / customBgImg.height;
                const destHeight = height - 380;
                ...
                ctx.drawImage(customBgImg, offX, 380 + offY, drawW, drawH);
            ...
            <div className="space-y-4 flex-grow mt-4 pr-1 min-h-0">
                ...
                {((report as any).license_plate || (report as any).vehicle_make || (report as any).vehicle_model || (report as any).vehicle_color || (report as any).year || (report as any).vin_number || (report as any).engine_number || (report as any).circulation_number) && (
                    <div className="grid grid-cols-2 gap-4 p-3 bg-gray-100 dark:bg-gray-800 rounded-lg">
                        ...
                        {(report as any).vin_number && <DetailField label="VIN Number">{(report as any).vin_number}</DetailField>}
                        {(report as any).engine_number && <DetailField label="Engine Number">{(report as any).engine_number}</DetailField>}
                        {(report as any).circulation_number && <DetailField label="Circulation Number">{(report as any).circulation_number}</DetailField>}
                ...
                {report.status === ReportStatus.RECOVERED && (report.type === 'vehicle' || report.type === 'crime') && (
                    <div className="grid grid-cols-2 gap-4 p-3 bg-gray-100 dark:bg-gray-800 rounded-lg">
                        ...
                        {(report as any).saps_13 && <DetailField label="SAPS-13">{(report as any).saps_13}</DetailField>}
                        {(report as any).pound_name && <DetailField label="Pound">{(report as any).pound_name}</DetailField>}
