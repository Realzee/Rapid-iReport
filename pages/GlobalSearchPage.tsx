...
const GlobalSearchPage: React.FC<{ profile: any; isGlobalAdmin: boolean }> = ({ profile, isGlobalAdmin }) => {
    ...
    const [activeTab, setActiveTab] = useState<'search' | 'portal'>('search');
    ...
    return (
        <div className="space-y-6">
            ...
            {activeTab === 'search' ? (
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col h-[75vh]">
                    <div className="bg-red-50 dark:bg-red-950/20 px-6 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center text-xs text-red-800 dark:text-red-400">
                        <span className="flex items-center gap-2">
                            <span className="relative flex h-2 w-2">
                              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                              <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
                            </span>
                            Natively communicating with external portal: 🚀 <strong className="font-semibold text-red-900 dark:text-red-100">https://rapidreportingsa.co.za/UltimateRegApp.html</strong>
                        </span>
                        <a 
                            href="https://rapidreportingsa.co.za/UltimateRegApp.html" 
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="bg-red-600 hover:bg-red-700 text-white font-medium px-4 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 text-xs shadow-sm"
                        >
                            Open in New Tab
                        </a>
                    </div>
                    <iframe 
                        src="https://rapidreportingsa.co.za/UltimateRegApp.html" 
                        className="w-full flex-grow border-0 rounded-b-xl"
                        title="UltimateReg Portal"
                    />
                </div>
            )}
