...
async function runMigrations() {
    ...
    const queries = [
        ...
        `CREATE TABLE IF NOT EXISTS public.ems_dispatches (
            ...
            triage_level text NOT NULL DEFAULT 'P2',
        ...
        "CREATE POLICY \"Allow insert ems_dispatches for authenticated\" ON public.ems_dispatches FOR INSERT TO authenticated WITH CHECK (true);",
        "DROP POLICY IF EXISTS \"Allow update ems_dispatches for authenticated\" ON public.ems_dispatches;",
        "CREATE POLICY \"Allow update ems_dispatches for authenticated\" ON public.ems_dispatches FOR UPDATE TO authenticated USING (true) WITH CHECK (true);",
        "DROP POLICY IF EXISTS \"Allow delete ems_dispatches for authenticated\" ON public.ems_dispatches;",
        "CREATE POLICY \"Allow delete ems_dispatches for authenticated\" ON public.ems_dispatches FOR DELETE TO authenticated USING (true);",
        ...
        "CREATE POLICY \"Allow insert ems_assessments for authenticated\" ON public.ems_assessments FOR INSERT TO authenticated WITH CHECK (true);",
        "DROP POLICY IF EXISTS \"Allow update ems_assessments for authenticated\" ON public.ems_assessments;",
        "CREATE POLICY \"Allow update ems_assessments for authenticated\" ON public.ems_assessments FOR UPDATE TO authenticated USING (true) WITH CHECK (true);",
        ...
        "UPDATE public.companies SET allowed_modules = ARRAY['controller', 'ems_dispatch', 'tech_ops', 'guard_monitoring', 'gate_access', 'attendance', 'analytics', 'archives'] WHERE allowed_modules IS NULL;",
        "UPDATE public.companies SET allowed_modules = array_remove(allowed_modules, 'fleet_management') WHERE allowed_modules IS NOT NULL AND ('fleet_management' = ANY(allowed_modules));",
        "UPDATE public.companies SET allowed_modules = array_append(allowed_modules, 'ems_dispatch') WHERE allowed_modules IS NOT NULL AND NOT ('ems_dispatch' = ANY(allowed_modules));",
...
const apiDir = process.env.NODE_ENV === 'production' 
    ? path.resolve(currentDir, 'dist', 'api', '_api_handlers') 
    : path.resolve(currentDir, 'api', '_api_handlers');
