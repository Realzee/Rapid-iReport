    ...
    if (req.method === 'POST') {
        ...
        if (action === 'fix-schema') {
            const queries = [
                ...
                "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'patrol_logs' AND policyname = 'Enable insert access for authenticated users') THEN CREATE POLICY \"Enable insert access for authenticated users\" ON public.patrol_logs FOR INSERT TO authenticated WITH CHECK (public.get_user_role(auth.uid()) IN ('admin', 'controller') OR exists (SELECT 1 FROM public.guards WHERE guards.id = patrol_logs.guard_id AND guards.profile_id = auth.uid())); END IF; END $$;",
                "NOTIFY pgrst, 'reload schema';"
            ];
            const results = [];
