...
CREATE POLICY "Allow staff to read ems assessments" ON public.ems_assessments FOR SELECT USING (public.get_user_role(auth.uid()) IN ('admin', 'moderator', 'controller', 'responder', 'ras_driver'));
